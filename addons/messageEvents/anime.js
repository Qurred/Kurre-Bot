...
function getAnimeById(client,msg,params){
  ...
  request.get(`https://${config.url}anime/${params[0]}?access_token=${config.token}`,(err,res,body) => {
    ...
    var res = new discord.RichEmbed()
      ...
      .setImage(response.image_url_banner ? response.image_url_banner: response.image_url_lge)
      .setDescription(response.description.replace(new RegExp('<br>', 'g'),''))
      .addField('Info', `${response.adult?'**This anime is for adults**\n':''}`+
      `Genres: ${response.genres.join(', ')}\n`+
      `${response.synonyms.length<1 ?"":'Also known as:\n' + response.synonyms.join('\n')}`+
      `Score: ${response.average_score}\n`+
      `Episodes: ${response.total_episodes}\n`+
      `Status: ${response.airing_status}\n`+
      `${response.youtube_id? '[Preview](https://www.youtube.com/watch?v=' + response.youtube_id+')\n':''}`
    );
